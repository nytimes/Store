...
И для социальных, и для новостных, и для business-to-business приложений, пользователи ожидают удобное взаимодействие как при подключении к сети, так и в автономном режиме.
...
Store похож на паттерн «репозиторий» [[https://msdn.microsoft.com/en-us/library/ff649690.aspx](https://msdn.microsoft.com/en-us/library/ff649690.aspx)], предоставляющий реактивный API, реализованный с помощью RxJava.
...
Когда вы создаёте новую реализацию Store, вы предоставляете ему `Fetcher` — функцию, которая определяет, как будет происходить загрузка данных из интернета.
...
Store возвращает данные в виде `Observable`, обеспечивая удобную работу с тредами.
Созданный Store управляет логикой обработки потока данных, позволяя вашему представлению использовать наиболее подходящий источник данных, и гарантирует, что новейшие данные будут доступны для последующего использования в автономном режиме.
...
Используя Store, с помощью двух слоёв кэширования (память и диск), вы исключите ситуации, когда один и тот же сетевой запрос будет выполняться избыточное количество раз.

### Полностью сконфигурированный Store
Для начала посмотрим, как выглядит полностью сконфигурированный Store.
Далее последуют более простые примеры, демонстрирующие каждую деталь в отдельности.
```java
...
С учетом указанной выше конфигурации вы получите:
...
+ Удобный API для запроса данных: получение кэшированных/новых данных или подписка на их будущие обновления.

А теперь более подробно:

### Создание Store

Для создания Store используется builder (строитель).
...
Store<ArticleAsset, Integer> store = StoreBuilder.<ArticleAsset,Integer>key()
        ...
        .open();
```
Store исползует универсальные ключи в качестве идентификаторов для данных.
...
Хороший вариант использования: пересоздание представления (activity или fragment) после поворота устройства — будут загружены кэшированные данные из Store.
Использование Store поможет вам избежать необходимости реализовывать эту логику на уровне представления.


Поток данных, которым управляет Store, будет выглядеть следующим образом:
...
Методы `fetch()` и `get()` одно значение и затем вызывает `onCompleted()` или выбрасывает исключение в случае ошибки.


### Поток
Для обновлений в реальном времени вы также можете вызвать метод `store.stream()`, который возвращает `Observable`, который производит эмиссию при сохранении нового объекта в Store.
...
Используйте оператор RxJava `filter()`, чтобы подписаться только на определенные события.
...
Store предлагает дополнительные механизмы для предотвращения дублирующих запросов одних и тех же данных.
Если некий запрос выполняется в течение минуты после предыдущего точно такого же запроса, возвращается тот же ответ.
Это полезно в ситуациях, когда вашему приложению требуется сделать много асинхронных вызовов для одних и тех же данных или когда пользователь интенсивно запрашивает обновление данных.
...
Обновленный поток данных будет выглядеть следующим образом:
...
Аналогичным образом есть артефакты парсеров для Moshi и Jackson!


### Кэширование на диске

Можно кэширование данных на диске, использую экземпляр класса `Persister` при создании Store.
...
Лучше, если данные будут передаваться из сети на диск с использованием BufferedSource или Reader в качестве типа данных (а не String).
...
Единственное требование заключается в том, что данные должны быть одного и того же типа при сохранении/получение, что и возвращаемое функцией Fetcher значение.
Технически, ничто не мешает вам реализовать с помощью класса `Persister` кэш в памяти (а не на диске), чтобы вместо двух слоёв кэша иметь один: с данными в оригинальном формате и уже распарсенными, что позволит делиться кэшем `Persister` между разными экземплярами Store.
...
Это позволяет функции получения данных работать непосредственно с сетевым потоком.
...
Мы установили, что наибольшей скорости сохранения на диск можно добиться, выполняя потоковое сохранени данных из сети.
Поэтому мы включили отдельный артефакт с библиотекой, предлагающей реактивную файловую систему, использующую `BufferedSource` из библиоткеи Okio.
...
+ Кэшировании в памяти с помощью Guava Cache
...
+ Возможность подписаться на все новые эмиссии из сети (метод `stream()`)
+ Возможность переподписки после очистки кэша (метод `getRefreshing()`. Полезно, если нужно выполнить запрос типа POST, после чего обновить экран)

Мы рекомендуем использовать эту конфигурацию для большинства хранилищ Store.
Благодаря передачи данных из сети на диск и с диска в парсер в формате потока байт, `SourcePersister` потребляет малое количество памяти.
Это позволяет нам загружать множество ответов в формате json размером более 1mb  и не беспокоиться об исплючениях OutOfMemory на устройствах с малым объемом памяти.
Как упоминалось выше, использование Store позволяет нам делать такие вещи, как вызов `configStore.get()` множествно раз асинхронно, прежде чем наша MainActivity закончит загрузку, не блокируя основной поток и не вызвая чрезмерной частоты сетевых запросов.
...
После этого, вы можете настроить работу Store один из двух способов:
...
`refreshOnStale` will backfill the disk cache anytime a record is stale.
The user will still get the stale record returned to them.
...
`networkBeforeStale` - Store will try to get network source when disk data is stale.
If the network source throws an error or is empty, stale disk data will be returned.
...
### Артефакты
Примечание: релизы синхронизированны с состоянием ветки master (а не develop)
...
+ **Cache** Кэш, извлеченный из библиотеки Guava (чтобы сократить количество методов)
	...
	```
+ **Store** Содержит только классы Store, зависит от RxJava и фртефакта кэша  
...
Кроме того, вики этого репозитория некоторые рецепты для распространненых сценариев использования 
