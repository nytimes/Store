...
public final class FileSystemRecordPersisterFactory {

  private FileSystemRecordPersisterFactory() {
  }
   ...
   * @throws IOException
   */
  @Nonnull public static <Key> Persister<BufferedSource, Key> create(@Nonnull File root,
      PathResolver<Key> pathResolver, long expirationDuration, @Nonnull TimeUnit expirationUnit)
      throws IOException {
    if (root == null) {
   ...
   * persistence {@link com.nytimes.android.external.fs.filesystem.FileSystem}.
   **/
  @Nonnull public static <Key> Persister<BufferedSource, Key> create(@Nonnull FileSystem fileSystem,
      PathResolver<Key> pathResolver, long expirationDuration, @Nonnull TimeUnit expirationUnit) {
    if (fileSystem == null) {
