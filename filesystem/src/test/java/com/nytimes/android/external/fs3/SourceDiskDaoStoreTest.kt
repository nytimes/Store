...
class SourceDiskDaoStoreTest {
    ...
    private val diskDAO: SourcePersister = mock()
    ...
    fun fetcherOnlyCalledOnce() = runBlocking<Unit> {
        ...
        whenever(diskDAO.read(barCode))
                .thenReturn(null)
                .thenReturn(source)

        whenever(diskDAO.write(barCode, source))
                .thenReturn(true)

        var result = store.get(barCode)
        assertThat(result.bar).isEqualTo(KEY)
        result = store.get(barCode)
        assertThat(result.bar).isEqualTo(KEY)
